...
class Dashboard extends BaseController
{
	...
	public function update($page_id)
	{
		...
		$data['page'] = $this->page->get($page_id);
		$data['access'] = json_decode($data['page']['access'], 1);
		...
		$report = $data['report'] = $this->report->get($data['page']['report_id']);
		...
		// dd($data);
		$data['result'] = $result ?? [];
		foreach ($currentCharts as $chart) {
		...
		$this->title = $data['page']['page_title'] . " adlı səhifənin redaktə edilməsi";
		$this->page("create", $data, false);
	}
}

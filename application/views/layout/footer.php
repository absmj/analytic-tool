  ...
  <!-- Loading -->
  <div id="loading" class="position-fixed w-100 h-100 top-0 d-flex justify-content-center align-items-center d-none" style="background-color: rgba(0, 0, 0, 0.55);z-index:281223">
    <div style="width: 200px; height: 200px" class="loading-cat">
  ...
  <script>
    ...
    language.load(localStorage.getItem('locale') || 'az', data => {
      window.locale = data;
      $("[data-i18n]").each(function() {
        if (['INPUT', 'TEXTAREA'].includes($(this).prop("tagName"))) {
          $(this).attr("placeholder", _.get(data, $(this).attr('data-i18n')))
        } else {
          if (!$(this).text()) {
            $(this).attr("title", _.get(data, $(this).attr('data-i18n')))
          } else {
            $(this).text(_.get(data, $(this).attr('data-i18n')))
          }
        }
      })
    })

...
const pageWizard = {
	...
	chartOptions(index) {
		return (
			this.charts.get(index).instance.apex.opts ||
			this.charts.get(index).instance.apex.options
		);
	},
...
$("[name='folder']").on("change", async (e) => {
	try {
		...
		if (!$("body").hasClass("toggle-sidebar")) $(".toggle-sidebar-btn").click();
		...
		if (!response.ok) {
			...
			uiInterface.error = file.message;
...
$("#action").on("click", ".file-explorer-file", async function () {
	try {
		...
		const response = await fetch(
			`${BASE_URL}files/get?file=${$(this).attr("file")}`
		);
		const file = await response.json();
		if (!response.ok) {
			uiInterface.error.serverSide = true;
...
$("[data-template]").on("click", async function () {
	try {
		...
		const request = await fetch(
			`${BASE_URL}dashboard/template/${$(this).attr("data-template")}`
		);
...
$("#writing").on("click", ".chart-type", async function () {
	try {
		...
		pageWizard.charts.set(uuid, {
			...
			options:
				dashboard.instances[
					$(this).parents("[data-action]").attr("data-target-modal")
				],
			slice: $(this).parent().attr("data-report-slice") || null,
		});

		$("#add-chart").attr({
			"data-affected-element": $(this)
				.parents("[data-action]")
		...
		chart.slice =
			typeof chart.slice == "object" ? chart.slice : JSON.parse(chart.slice);
...
function generateInputs(data, index, parentKey = "", defaults = null) {
	if (defaults === null) {
		const apexOptions = [
			...Object.getOwnPropertyNames(ApexOptions).filter(
				(prop) => typeof ApexOptions[prop] === "function"
			),
		];
		defaults = new Object();
	...
	for (let key in data) {
		...
		if (!parentKey)
			document
				.getElementById("chart-options-form")
				.insertAdjacentHTML(
					"beforeend",
					`<hr><h4 data-bs-toggle="collapse"  data-bs-target=".${inputId}" class="card-title pt-0 mt-0" role="button" aria-expanded="false" aria-controls="${inputId}">${key}</h4>`
				);
		...
		if (typeof value === "object") {
			...
			if (typeof value === "boolean") {
				...
				const switcherHTML = `
                    <div class="collapse form-check form-switch ${parentKey.replace(
											/(.*?)-.*/gm,
											"$1"
										)}">
                        <input onchange="changeState('${index}', '${inputId
					.replace(/-*(\w+)-*/g, ".$1")
					.replace(/\.*(\d+)/gm, "[$1]")}', this.checked ? true : false)" ${
					value ? "checked" : ""
				} class="form-check-input" type="checkbox" id="${inputId}">
                        <label class="form-check-label" for="${inputId}">${label}</label>
                    </div>
                `;
				document
					.getElementById("chart-options-form")
					.insertAdjacentHTML("beforeend", switcherHTML);
			} else if (/#([a-zA-Z0-9])/.test(value)) {
				const colorinput = `
                    <div class="collapse mb-3 ${parentKey.replace(
											/(.*?)-.*/gm,
											"$1"
										)}">
					...
					.replace(
						/\.*(\d+)/gm,
						"[$1]"
					)}', this.value)" type="color" class="form-control" id="${inputId}" value="${value}">
                    </div>
                `;
				document
					.getElementById("chart-options-form")
					.insertAdjacentHTML("beforeend", colorinput);
				...
				const rangeinput = `
                    <div class="collapse mb-3 ${parentKey.replace(
                        ...
                        <label for="${inputId}" class="form-label">${label}</label>
                        <input step="${
													value > 0 && value % 1 === 0 ? 0.1 : 1
												}" max="${
					value > 0 && value % 1 === 0 ? 1 : 1000
				}" oninput="changeState('${index}', '${inputId
					.replace(/-*(\w+)-*/g, ".$1")
					.replace(
						/\.*(\d+)/gm,
						"[$1]"
					)}', this.value)" type="range" class="form-range" id="${inputId}" value="${value}">
                    </div>
                `;
				document
					.getElementById("chart-options-form")
					.insertAdjacentHTML("beforeend", rangeinput);
			} else {
				...
				const inputHTML = `
                    <div class="collapse mb-3 ${parentKey.replace(
                        ...
                        <label for="${inputId}" class="form-label">${label}</label>
                        <input onfocusout="changeState('${index}', '${inputId
					.replace(/-*(\w+)-*/g, ".$1")
					.replace(
						/\.*(\d+)/gm,
						"[$1]"
					)}', this.value)" type="text" class="form-control" id="${inputId}" value="${value}">
                    </div>
                `;
				document
					.getElementById("chart-options-form")
...
$("#add-chart").on("click", function () {
	...
	const el = $($(this).attr("data-affected-element"))
		.parents("[data-action]")
		.find(".chart-type");
	el.addClass("btn-light");
	$($(this).attr("data-affected-element"))
		.parents("[data-action]")
		.find(`.chart-type[data-type="${pageWizard.charts.get(uuidBtn).type}"]`)
	...
	dashboard.updateInstance(
		$(this).attr("data-affected-element"),
		pageWizard.chartOptions(uuidBtn)
	);
	chartWizard.hide();
});
...
$("#main").on("click", "#save-page", async function () {
	try {
		...
		if (pageWizard.chartPost.length == 0) {
			throw new Error(
				"Ən azı bir çart yaradılmalıdır. Nümunədəki çartlar səhifə yaratmaq üçün əsas sayılmır"
			);
		}
